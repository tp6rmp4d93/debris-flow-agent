...
with col_btn1:
    if st.button("載入南部多組別範例 (12 條)", key="btn_sample_south"):
...
with col_btn2:
    if st.button("載入相鄰縣市自駕範例 (5 條)", key="btn_sample_adjacent"):
...
stream_input_text = st.text_area("請以逗號或換行分隔溪流編號：", value=default_text, height=75, key="input_streams_area")
...
num_groups = col_g1.selectbox("分組數量", [1, 2, 3, 4], index=0, key="select_num_groups")
...
if num_groups > 1:
    ...
    for i in range(num_groups - 1):
        ...
        cnt = g_cols[i].number_input(f"{group_labels[i]}組 分配數量", min_value=0, max_value=rem, value=def_val, step=1, key=f"num_grp_{i}")
        group_counts[group_labels[i]] = cnt
        rem -= cnt
    last_grp = group_labels[num_groups - 1]
    group_counts[last_grp] = rem
    g_cols[num_groups - 1].text_input(f"{last_grp}組 分配數量 (自動計算)", value=str(rem), disabled=True, key=f"num_grp_auto_{num_groups}")
else:
...
start_date = col_cond1.date_input("現勘起始日期", value=datetime.date.today(), key="input_start_date")
leader_info = col_cond2.text_input("領隊電話 / 各組聯絡人資訊", value="賴承農 0963", key="input_leader_info")
...
# 6. 介面流程：步驟三 (開始排程與預覽)
# ==============================================================================
if st.button("🚀 確認無誤，開始多組別智慧排程", type="primary", use_container_width=True, key="btn_start_schedule"):
    ...
    for grp in active_groups:
        ...
        s_data, d_routes, t_mode = run_schedule_simulation(
            grp_streams, 
            st.session_state["start_locations_dict"].get(grp, []), 
            st.session_state["start_times_dict"].get(grp, []), 
            start_date, grp
        )
...
if "schedule_data_dict" in st.session_state and st.session_state["schedule_data_dict"]:
    ...
    for grp in sorted(st.session_state["schedule_data_dict"].keys()):
        ...
        s_data = st.session_state["schedule_data_dict"].get(grp, [])
        d_routes = st.session_state["daily_routes_dict"].get(grp, [])
        ...
        with col_v2:
            ...
            st.write(st.session_state.get("transit_mode_dict", {}).get(grp, "系統判定中..."))
        ...
        for route in d_routes:
            ...
            if len(pts) >= 2:
                ...
                with col_r2:
                    st.link_button("📍 開啟導航", gmaps_url, use_container_width=True, key=f"btn_nav_{grp}_{route['day']}")
        st.markdown("<br>", unsafe_allow_html=True)

    if st.button("🔄 套用各組自訂排序並全面重算", type="primary", use_container_width=True, key="btn_recalc_all"):
        for grp, ed_df in edited_sort_dfs.items():
            sorted_indices = ed_df.sort_values(by="自訂排序").index
            current_streams = st.session_state.get("current_parsed_streams_dict", {}).get(grp, [])
            ...
            new_schedule, new_routes, new_transit = run_schedule_simulation(
                reordered_streams, 
                st.session_state.get("start_locations_dict", {}).get(grp, []), 
                st.session_state.get("start_times_dict", {}).get(grp, []), 
                start_date, grp
            )
    ...
    with col_exp2:
        if st.button("☁️ 儲存規劃紀錄至 Turso 雲端", type="secondary", use_container_width=True, key="btn_save_turso"):
                ...
                stmts = [{
                    ...
                    "stmt": {
                        ...
                        "args": [
                            ...
                            {"type": "integer", "value": str(max([len(r) for r in st.session_state.get("daily_routes_dict", {}).values()] + [0]))}
                        ]
                    }
                }]
                    ...
                    stmts.append({
                        ...
                        "stmt": {
                            ...
                            "args": [
                                ...
                                {"type": "integer", "value": str(item.get("day_no", 1))},
                                {"type": "integer", "value": str(item.get("項次", 1))},
                                {"type": "text", "value": item.get("編號", "")},
                                {"type": "text", "value": item.get("縣市", "")},
                                {"type": "text", "value": item.get("鄉鎮市區", "")},
                                {"type": "text", "value": item.get("會合時間", "")},
                                {"type": "text", "value": item.get("location_name", "")}
                            ]
                        }
                    })
...
uploaded_json = st.file_uploader("匯入由調查系統產出之 stream_database.json 更新歷史會合點主庫：", type=["json"], key="uploader_json")
if uploaded_json:
    try:
